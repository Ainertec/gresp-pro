...
async function getTotal(products, drinkables) {
  ...
  for (const element of products) {
    ...
    if (productsTemporary)
      productsTotal += (productsTemporary.price * element.quantity);
  }
  for (const element of drinkables) {
    ...
    if (drinkableTemporary)
      drinkableTotal += (drinkableTemporary.price * element.quantity);
  }
...
module.exports = {
  ...
  async update(req, res) {
    ...
    try {
      const orderUpdated = await Order.findOneAndUpdate({ identification, "closed": false }, {
        identification,
        products,
        drinkables,
        total: finalPrice.toFixed(2),
  ...
  async destroy(req, res) {
    ...
    try {
      const order = await Order.findOneAndUpdate({ identification, "closed": false }, { "closed": true, "payment": payment });
